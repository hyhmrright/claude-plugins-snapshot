...
2. **Startup delay**: Waits 10 seconds to let Claude Code finish initialization (`async: true` delegates backgrounding to Claude Code, avoids race condition where `claude plugin update` fails because the plugin system isn't ready)
...
7. **Session detection**: Checks `CLAUDECODE` environment variable
   ...
   - `session-start.sh` unsets this variable before execution
