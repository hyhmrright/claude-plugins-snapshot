...
1. **Hook Layer** (Dual Guarantee)
   ...
   - Both call `scripts/session-start.sh` for background execution (avoids blocking startup), with 10-second delay to wait for Claude Code initialization
...
2. **Startup delay**: Waits 10 seconds in background to let Claude Code finish initialization (avoids race condition where `claude plugin update` fails because the plugin system isn't ready)
3. **Backup cleanup**: Auto-deletes timestamped config backup files generated by Claude Code
